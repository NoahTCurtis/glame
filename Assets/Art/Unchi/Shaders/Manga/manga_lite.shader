...
#include "UnityCG.cginc"
      ...
      fixed4 frag (VS i): COLOR {
        ...
        if (m > 0.7) {
          fixed f = clamp((m - 0.7) * _ToneBlend, 0, 1);
          o = f + tex2D (_Tone1Sampler, pos).r * (1 - f);
        } else
        if (m > 0.4) {
          fixed f = clamp((m - 0.4) * _ToneBlend, 0, 1);
          o = tex2D (_Tone1Sampler, pos).r * f + tex2D (_Tone0Sampler, pos).r * (1 - f);
        } else
        if (m > 0.1) {
          fixed f = clamp((m - 0.1) * _ToneBlend, 0, 1);
          o = tex2D (_Tone0Sampler, pos).r * f;
        }
