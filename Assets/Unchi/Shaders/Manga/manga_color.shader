...
//        fixed m = (0.299 * c.r + 0.587 * c.g + 0.114 * c.b) * i.Shadow;
        ...
        if (m > 0.75) {
          fixed f = clamp((m - 0.75) * _ToneBlend, 0, 1);
          o = f + tex2D (_Tone2Sampler, pos).r * (1 - f);
        } else
        if (m > 0.50) {
          fixed f = clamp((m - 0.50) * _ToneBlend, 0, 1);
          o = tex2D (_Tone2Sampler, pos).r * f + tex2D (_Tone1Sampler, pos).r * (1 - f);
        } else
        if (m > 0.25) {
          fixed f = clamp((m - 0.25) * _ToneBlend, 0, 1);
